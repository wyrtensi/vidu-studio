...
if (document.body.getAttribute('data-wyrtensi-vidu-studio-injected') === 'true') {
  ...
  function waitForElement(selector, timeout = 10000) {
    return new Promise((resolve, reject) => {
      const interval = 1000; // Increased to 1000ms
  ...
  async function waitForCondition(conditionFn, interval = 1000, timeout = 10000) { // Increased interval to 1000ms
    return new Promise((resolve, reject) => {
      const startTime = Date.now();
  ...
  async function findUploadWrapper() {
    try {
      ...
      await waitForCondition(() => uploadContainer.querySelectorAll('.group').length === 3, 1000, 5000);
      const slots = uploadContainer.querySelectorAll('.group');
      if (slots.length !== 3) {
        console.warn(`Expected exactly 3 slots, found ${slots.length}.`);
        return null;
      }
  ...
  function setupDragAndDrop(uploadWrapper, uploadSlots) {
    ...
    uploadWrapper.addEventListener('drop', (event) => {
      ...
      distributeFilesToSlots(files, uploadSlots);
    });
    ...
    uploadSlots.forEach(slot => {
      ...
      slot.addEventListener('drop', (event) => {
        ...
        slot.classList.remove('dragover');
        const files = event.dataTransfer.files;
        distributeFilesToSlots(files, uploadSlots, slot);
      });
    });
  ...
  function setupClickToUpload(uploadWrapper, uploadSlots) {
    ...
    multiFileInput.addEventListener('change', () => {
      ...
      distributeFilesToSlots(files, uploadSlots, targetSlot);
      setTimeout(() => {
  ...
  async function setupUploadFunctionality() {
    try {
      if (!isOnReferenceToVideoTab() || isSetupDone) return;
      ...
      if (uploadSlots.length !== 3) return;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    if (isOnReferenceToVideoTab()) debounceSetup();
  });
  ...
  const debouncedObserverCallback = debounce(() => {
    if (isOnReferenceToVideoTab() && !isSetupDone) debounceSetup();
  }, 500);
